...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Check if email is verified - only if not using 2FA
        if (!user.emailVerified && !user.isTwoFactorEnabled) {
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      // For credentials, we've already checked in the authorize callback
      return true
    },
